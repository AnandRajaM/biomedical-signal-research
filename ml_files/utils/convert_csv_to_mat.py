...
# Create data_dict in the exact order from mat files
data_dict = {
    '__header__': mat_data['__header__'],
    '__version__': mat_data['__version__'],
    '__globals__': mat_data['__globals__'],
    'fs_bp': mat_data['fs_bp'].astype(np.float64),  
    'fs_ecg': mat_data['fs_ecg'].astype(np.float64),  
    'fs_icg': mat_data['fs_icg'].astype(np.float64),  
    'fs_intervention': mat_data['fs_intervention'].astype(np.float64),  
    'fs_radar': mat_data['fs_radar'].astype(np.float64),  
    'fs_z0': mat_data['fs_z0'].astype(np.float64),  
    ...
    'tfm_ecg1': mat_data['tfm_ecg1'].astype(np.float64) if 'tfm_ecg1' in mat_data else np.array([]).reshape(-1, 1),  
    'tfm_ecg2': mat_data['tfm_ecg2'].values.reshape(-1, 1).astype(np.float64) if 'tfm_ecg2' in csv_data.columns else np.array([]).reshape(-1, 1),
